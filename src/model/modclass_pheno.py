...
# ------------------------------------------------------------------------------
    ...
    def day_bb(self, d_onset):
        """
        Calculates day of bud-burst for each year of data using cumulative temperature.
        ...
        for y in np.unique(self.dC.year):
            ...
            idx = year_ind[0]
            day_on = np.where(np.cumsum(self.dC.t_mean[idx:]) > t_bb)[0][0]
            daybb_arr[idx:idx+len(year_ind)] = np.ones(len(year_ind))*day_on
        return daybb_arr
    ...
    def dalecv2(self, p):
        ...
        phi_on = self.phi_onset(self.daybb_arr[self.x], p[13])
    ...
    def mod_list(self, pvals):
        ...
        """
        self.daybb_arr = self.day_bb(pvals[11])
    ...
    def evolve_mat(self, mat, matlist):
        """
        Evolves matrix using list of matrices provided
        :param mat: matrix to be evolved
        :param matlist: list of matrices to evolve matrix.
